...
if __name__ == '__main__':
    ...
    ax[0].set_title('Received Data; Prior to iq_find')
    ax[0].legend()

    tst = iq_find(rx_debg,fc,fs)
    t2 = np.linspace(0,len(tst)/fs,len(tst))
    ax[1].plot(t2,np.real(tst),label='In-Phase')
    ax[1].plot(t2,np.imag(tst),label='Quadrature')
    ax[1].set_title('Received Data; Post iq_find')
    ax[1].legend()
